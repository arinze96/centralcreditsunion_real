...
function loadOpen(a) {
  ...
  $.post(param, function(res) {
    ...
    var nCh = $('<label>').css({'float': 'left', 'margin-top': '2.2rem'}).addClass("col s1").appendTo(hd);
    ...
    $.each(result.row, function(i, field) {
      ...
      var nCs = $('<label>').css({'float': 'left'}).addClass('col s1').click(function(e) {
        e.stopPropagation();
      }).appendTo(nD);
      $('<input>').attr({
        'type': 'checkbox',
        'id': 'cbx_' + field.i + a,
        'value': field.i
      }).appendTo(nCs).addClass('cbx filled-in');
      $('<span>').css({
        'float': 'left'
      }).addClass("oRow-numbering").text($recordstart + '. ').appendTo(nCs);
