...
$.fn.extend({
  uploadDialog: function (init) {
    ...
    if (!$("#upload_modal")[0]) {
      var mod = $("<div>")
        .addClass("modal row modal-fixed-footer")
        .css("width", "80%")
        .attr({ id: "upload_modal" })
        .appendTo("body");
      var mod_content = $("<div>")
...
function createDivs(a, type) {
  ...
  var fcol1 = $("<div>")
    .appendTo(fRow)
    .addClass("col s12 m2 file-field input-field mtop1");
  var filebtn = $("<div>")
  ...
  var uc_srch = $("<div>")
    .addClass("col s4 m1 pos_relative mtop1")
    .css({ "z-index": "20" })
    .appendTo(fRow);
  var uc_srcha = $("<a>")
...
function resourceModal_2(type, name) {
  if ($("#resourceModal")[0] == null) {
    ...
    var cont_h4 = $("<div>")
      .addClass("col s12 m5 elu_name")
      .css({ overflow: "hidden" })
      .appendTo(imgmod_cont);
    var cont_sty = $("<div>")
      .addClass("col s12 m7 attributes")
      .appendTo(imgmod_cont);
    var c_cl = $("<div>")
...
function newCard(prt, file) {
  ...
  var an = $("<a>")
    .appendTo(prt)
    .addClass("col s6 m3 l2 uploadFolders")
