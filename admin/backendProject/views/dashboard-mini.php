...
if(isset($_POST['getChart'])){
	...
	foreach($row as $type => $info){
		...
		for($i=1; $i<=12;$i++){
			...
			foreach($info as $m => $month ){
				if($month['month'] == $i){$nums[$i]=intval($month['num']);}
			}
		}
  ...
  $pla = "SELECT name, plan, count(plan) as num FROM accounts GROUP BY plan ORDER BY num DESC" ;
...
$(document).ready(function(){
	$.post(`${site.backend}custom-view/dashboard-mini`,{'getChart':true},function(response){
		var ctx = $('#myChart');
