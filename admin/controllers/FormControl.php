...
class FormControl{
			...
			foreach($coldesc as $_k0 => $v0) { ?>
			 <div class="page">
				<?php foreach($v0 as $_k => $v) {
					...
					<div <?php if($_k == 'modal') echo 'class="modal" id="'.$modal_id.'"';else echo 'class="'.trim(explode(' ', $_k)[0]).'desc"' ?>>
				  		...
				  		<div class="row ">
								<div class="col s12">
									...
									<div class="card hoverable" >
						     		...
						     		<?php if(!empty($_k1)) {?>
							     			<div class="card-title black-text"><?=$_k1; ?></div>
										<?php } ?>
										<?php
  ...
  public function create_elements($vt, $_k0, $_k, $_k1, $ex='', $param=null){
		...
		foreach($v1 as $_k2=>$desc) {
			...
			if($type=="text"||$type=="url"||$type=="tel"||$type=="password"||$type=="date"||$type=="email"||$type=="number"||$type=="hidden" || $type=="button" || $type=="combo" || $type=="textarea" || $type=="time"||
			$type=="richtext-title"){ ?>
				<div class="input-field <?=$class; ?>">
					<?php if($type == "textarea" || $type == "button"){?><<?=$type?> <?php }else{?> <input <?php } ?>
						<?php if($Disabled==true) {?> disabled="disabled" <?php }?>
						<?php if($Readonly==true){?>readonly="readonly"<?php } ?>
						<?php if(!empty($Event)){?>
							<?=$Event["type"]?>="<?=$Event["function"]?>"
						<?php }?>
						<?php if($multi != true){?> browser-default	<?php }?>
						<?php if($Required==true) { ?>required="required"<?php }?>
						<?php if($type=='number') { ?>step="any"<?php }?>
						<?php if($type=='combo') { ?> display_fields="<?=$multi?>" insert_field = "<?=$Value?>" <?php }?>
						<?php if($type!='combo') { ?>value="<?=$Value?>"<?php }?>
						<?php if($type!='combo' && strlen($Value)) { ?>data-keep="<?=$Value?>"<?php }?>
						data-source="<?=$source?>"
						name="<?=$name.$ex; ?>"
						id="<?=$name.$ex; ?>"
						type="<?=$type?>"
						class="filled-in <?="{$type} {$name} {$keep}"?>
						<?php if(strlen($Value) && $type!=='combo'){?>keep<?php }?>"
						<?php if(!empty($ex)){ ?>data-class="<?=$name; ?>" <?php }?>
						<?php if(!empty($plh)){ ?>placeholder="<?php if($Required==true) { ?>*<?php }?><?=$desc?>" <?php }?>
					>
			...
			<?php }else if($type=="radio") { ?>
				<div class="input-field <?=$class ?>">
					<div class="left" style="height:auto">
						...
						foreach ($buttons as $value => $desc) {?>
							<label class=left>
								<input
								<?php if($Disabled==true) {?> disabled="disabled" <?php }?>
								<?php if($Readonly==true){?>readonly="readonly"<?php } ?>
								<?php if($Required==true){?>required="required"<?php } ?>
								class="filled-in keep"
								type="radio"
								value="<?=$value; ?>"
								data-source="<?=$source?>"
								name="<?=$name.$ex; ?>"/>
			...
			<?php } else if($type=="file") { ?>
				<div class="file-field input-field left <?=$class ?>">
				  <div
						class="btn generic-file-selector"
						data-href ="<?=$this::$uri->backend?>processors/process_upload.php"
						data-path ="<?=absolute_filepath($this::$uri->site)?>/assets/"
						data-edit ="0"
						data-display ="<?php if(empty($Value)){?> all <?php }else{ echo $Value; } ?>"
			...
			<?php } else if($type=="select") {?>
				<div class="input-field <?=$class ?>">
					<select
						class="<?php if($multi != true){?> browser-default	<?php }?> <?=$name?> <?=$keep?>"
						data-value="<?=$Value?>"
						data-empty="<?=$Empty?>"
						data-source="<?=$source?>"
						name="<?=$name.$ex; ?>"
						id="<?=$name.$ex; ?>"
						<?php if($multi == true){?> multiple="multiple"	<?php } ?>
						<?php if($Required==true) { ?> required="required" <?php } ?>
						<?php if($Disabled==true) { ?> disabled="disabled" <?php } ?>
						<?php if(!empty($event[$_k0][$_k][$_k1][$_k2])){
							$_selFunc = empty($Event['function']) ? 'heirachy_select(this)': $Event['function'];
							$Event['type'] = empty($Event['type']) ? "onchange" : $Event['type'];?>
							<?=$Event['type']?>="<?=$_selFunc?>"
							<?php if(!empty($Event['target'])){?>
								data-target="<?=$Event['target']?>"
								data-target-source="<?=$this::$paramControl->source_encode($Event['source'])?>"
								<?php if(!empty($Event['action'])){?> data-target-action="<?=$Event['action']?>"	<?php } ?>
							<?php } ?>
						<?php }?>
					>
					<?php
		  ...
		  <?php } else if($type=="switch") { ?>
			  <div class="input-field <?=$class ?>">
				 ...
				 $vy = [];
				 foreach($data as $_k3=> $v3) { $_ky[]=$_k3; $vy[]=$v3; } ?>
				 <label for="<?=$name; ?>" class="active"><?php if($Required==true) { ?>*<?php }?><?=$desc?></label>
				 <div class="switch" style="height:50px">
					 <label>
							...
							<?php if($Disabled==true) {?> disabled="disabled" <?php }?>
							<?php if($Readonly==true){?>readonly="readonly"<?php } ?>
							<?php if(strlen($Value)){?>checked="checked"<?php }?>
							<?php if($Required==true){?>required="required"<?php } ?>>
		...
		<?php } else if($type=="buttons") {
			...
			<?php } else if($type=="slave") { ?>
					...
					<div class="input-field">
						<div class="checkbox-container" style="position:relative; width:100%">
							<label for='<?=$name.$ex?>'>
								<input
								<?php if($Disabled==true) {?> disabled="disabled" <?php }?>
								<?php if($Readonly==true){?>readonly="readonly" <?php } ?> name="<?=$name.$ex; ?>"
								id="<?=$name.$ex; ?>"
								<?php if($Required==true) { ?>required="required"<?php }?>
								type="checkbox" value="<?=$Value?>"
								class="filled-in <?=$name?> <?php if(strlen($Value))echo 'keep' ?>"
								<?php if(!empty($ex)){ ?>data-class="<?=$name; ?>" <?php }?>
								onclick="toggleSlave(this)"
								/>
				...
				<?php }else if($type=="displaypicture") { ?>
							<div class="displaypicture <?= $class?>" data-name="<?=$desc?>" ><img width="100%" name="<?=$name; ?>" id="<?=$name; ?>" src=""></div>
			  <?php }
				...
				<?php } else if($type=="items") { ?>
					<div data-num="<?=$Value?>" class="items-container <?php if($Required==true) echo "required ";echo $class ?>" data-desc="<?=$desc?>" name="<?=$name; ?>" >
					</div>
					<!-- JSON MANAGER WIDGET -->
				<?php } else if($type=="json-manager") { ?>
					<div class="json-container <?php if($Required==true) echo "required ";echo $class ?>" data-source="<?=$source?>" data-desc="<?=$desc?>" name="<?=$name;?>" >
					</div>
					<!-- Form Builder -->
				<?php } else if($type=="form-builder") { ?>
					<div class="form-builder <?php if($Required==true) {echo "required "; echo $class; }?>" data-desc="<?=$desc?>" name="<?=$name;?>" >
					</div>
					<!-- Generic Slider -->
				...
				<?php } else if($type=="dynamic-lists") { ?>
					...
					<div class="<?=$class ?> list-container" name="<?=$name;?>"
						<?php if(!empty($Event)){?>
						<?=$Event["type"]?>="<?=$Event["function"]?>"
						<?php }?>
					>
				...
				<?php } else if($type=="picture") { ?>
					<div class="input-field <?= $class?>">
						<a
						href="javascript:;"
						data-href ="<?=$this::$uri->backend?>process/dialog"
						data-path ="<?=absolute_filepath($this::$uri->site)?>assets/"
						class="<?=$name; ?>"
						data-display="picture"
						id="uploadPic"
					>
						<img src="<?php if(isset($col_row[$name]))echo $col_row[$name];else echo $this::$uri->backend.'images/default.png';?>" style="width: 100%;" class='form_pic' data-name="<?=$name; ?>">
  ...
  public function create_static_elements($vt, $_k0, $_k, $_k1, $ex='', $param=null){
			...
			?>
				<div class="">
					<ul class="collection no-collection">
						<?php $count = 1; foreach ($array as $key => $arrayvalue):
	...
	public function build_action_modal($thisparam, $param){
		...
		<form
		 ...
		 data-action="<?=$thisparam->process_url?>"
		 method="post"
		 id="fm_action_<?=strToFilename("{$param->page_title} {$thisparam->page_title}")?>"
		 data-submit="<?=$thisparam->submit_type?>"
	...
	public function build_report_modal($thisparam, $param){
      ...
      "date" => "Date Range",
		...
		?>
		<div class="modal-title"> <h4 class="center-align pt-2 mb-0"><?=$thisparam->page_title ?></h4>  </div>
			...
			<div class="row">
				...
				<div class="col s8 panel-div" id="panel-div">
					<div class="row active" name="float_div" id="subdiv_column">
						<p class="my-0">Display Columns<hr> </p>
					...
					<?php foreach ($sections as $section_key => $section_title): ?>
						<div class="row" name="float_div" id="subdiv_<?=$section_key?>" style="display:none">
							<p class="my-0"><?=$section_title ?> <hr> </p>
							<div class="col s12">
								<!-- ==================================================== Date Range section -->
								...
								<?php elseif(in_array($section_key, $report_setup->sections) && $section_key == "grouping"):
									...
									<div class="col s12">
										...
										<ul class="select-data">
											<?php foreach (array_filter($report_setup->display_fields, function ($value) { return isset($value->graph); }) as $i => $list): ?>
												<li class="left w-50">
