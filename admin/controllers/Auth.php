...
class Auth extends Generic {
	...
	public function __construct($get_user, $param_name = "admin_signin"){
		...
		if($get_user !== null){
			...
			if($param_name == "admin_signin"){$required_cols = array_merge($required_cols, $required_admin);}
	...
	public function user($get_user = null){
		...
		if(empty(self::$user) && !empty($identity_val)){
			...
			//see("$ucol=$identity_val");
			$user = $this->getFromTable($column_maps->table, "$ucol=$identity_val", 1, 1);
			if(count($user)){
				$user = $user[0];
				$user->roledesc = "";
				if(!empty($user->roleid))
				$user->roledesc = $this->getFromTable("roles","roleid={$user->roleid}", 1, 1, "", false, "roledesc")[0]->roledesc;
				...
				$user->image_col 	= file_path($user->image_col, $this->domain.$this->backend."images/default.png");
				self::$user = $user;
			}else{
				$user = null;
			}
    ...
    if(!empty($param)) {
				...
				if (!empty($param->authy) && $param->authy == "SMS") {
					...
					if(!empty($user->email_col) && !empty($company->email)){
						$data = [
							...
							'to_name'	=>	$user->name_col || $user->email_col,
							'template'	=>	"token",
							'token'	=>	$code
						];
			...
			}else{$response->message = "Not A registered User";}
		}else{$response->message = "The formName you supplied is invalid | does'nt have paramters";}
	    return($response);
  }
    ...
    $param        = self::$column_maps;
		...
		if(!empty($param) && !empty($user)) {
			if(!empty($_SESSION["code"])){
				...
				}else{$response->message = "Token is incorrect";}
			}else{$response->message = "Token has expired";}
		}
		return $response;
  }
  ...
  public function login($data) {
    ...
    if(!empty($param)) {
      ...
      if(!empty($user)) {
        ...
        }else { $response->message = "Password does not match"; }
      } else { $response->message = "User not found, Try Creating an Account"; }
    }else{ $response->message = "The formName you supplied is invalid | does'nt have paramters"; }
    return $response;
  }
}
